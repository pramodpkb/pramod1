...
function initCitybook() {
    ...
    $(".scroll-book  a ").singlePageNav({
        filter: ":not(.external)",
        updateHash: false,
        offset: 160,
        threshold: 150,
        speed: 1200,
        currentClass: "act-scrlink-1"
    });
    $("#btnbookDet").click(function () {
        $('html, body').animate({
    ...
    var forgotmodal = {};
    forgotmodal.hide = function () {
        $('#forgotmodal , .reg-overlay').fadeOut(200);
        $("html, body").removeClass("hid-body");
    };
    ...
    $('.Forgot-modal-open').on("click", function (e) {
        ...
        modal.hide();
        e.preventDefault();
        $('#forgotmodal , .reg-overlay').fadeIn(200);
        $("html, body").addClass("hid-body");
    });
    ...
    //$('input[name = "header-search"]').val();
    $('input[name="header-search"]').daterangepicker({
        autoUpdateInput: false,
        "startDate": $('#servermindate').val(),
        "endDate": $('#servermindate').val(),
        "minDate": $('#servermindate').val(),
    ...
    $('input[name="header-search"]').on('apply.daterangepicker', function (ev, picker) {
        ...
        if (picker.startDate.format('DD/MM/YYYY') != picker.endDate.format('DD/MM/YYYY')) {
            document.getElementsByName('Selectslot')[0].value = 4;
            $('.header-search-input-item').find('li').each(function (t, u) {
                if (u.dataset.value != 4)
                    $(u).removeClass('selected');
                else
                    $(u).addClass('selected');
            })
            $('.header-search-input-item').find('span[class="current"]')[0].innerHTML='24 Hrs'
             
        }
        else {
            document.getElementsByName('Selectslot')[0].value = 1;
            $('.header-search-input-item').find('li').each(function (t, u) {
                if (u.dataset.value != 1)
                    $(u).removeClass('selected');
                else
                    $(u).addClass('selected');
            })
            $('.header-search-input-item').find('span[class="current"]')[0].innerHTML = '3 Hrs'
        }
    });
    ...
    $('input[name = "main-input-search"]').val($('#servermindate').val() + ' - ' + $('#servermindate').val());
    ...
    if ($(".fixed-bar").outerHeight(true) < $(".post-container").outerHeight(true)) {
        ...
        $(".fixbar-action").scrollToFixed({
            minWidth: 1064,
            marginTop: function () {
                var a = $(window).height() - $(".fixed-bar").outerHeight(true) - 100+53;
                if (a >= 0) return 40;
                return a;
            },
            removeOffsets: true,
...
var roomsval = $('div[id*="SearchBox"]').length, childsval;
var RoomsClone = $('#SearchBox1').clone();
$('.roomsQua').on('change', function (e) {
    if (roomsval != e.target.value) {
        if (roomsval < e.target.value) {
            roomsval = e.target.value
            var ele = $(RoomsClone).clone();
            $(ele).find('.roomcopy').html('<span class="roomnos">Room ' + e.target.value+'</span>');
            ele[0].id = 'SearchBox' + e.target.value
            $(ele).css('margin-top','4px')
            $('#CommonSearch').append(ele);
            $('.childQua').off('change');
            BindChilds();
...
BindChilds();
function BindChilds() {
    $('.childQua').on('change', function (e) {
        var par = $(e.target).closest('div[id*="SearchBox"]');
        var childcheck = $(par).find('input[data-id*="childAge' + e.target.value + '"]');
        if (e.target.value == 0)
            $(par).find('div[class*="childAge"]').remove();
        else if (e.target.value==1)
        {
            $(par).find('div[class*="childAge"]').not('div[class*="childAge' + e.target.value + '"]').remove();
        }
        if (childcheck.length == 0 && e.target.value != 0) {

            $(par).append('<div class="quantity-item childAge' + e.target.value + '" style="width:auto !important"><label>Child ' + e.target.value + '</label><div class="quantity"><input class="childage" data-id="childAge' + e.target.value + '" readonly type="number" min="1" max="12" step="1" value="1"></div></div>');

            $('<div class="quantity-nav"><div class="quantity-button quantity-up">+</div><div class="quantity-button quantity-down">-</div></div>').insertAfter($(par).find(".quantity input").last());
            $(par).find(".quantity").last().each(function () {
                var t = jQuery(this),
                    i = t.find('input[type="number"]'),
                    n = t.find(".quantity-up"),
                    a = t.find(".quantity-down"),
